...
> 작성 기준: **v158 작업 브랜치** (2026-08). 앱 라이브 기준본은 `main`이며, 디자인 작업은 Draft PR 브랜치에서 진행합니다.
...
- **현재 목표**: 실제 `🐾 육성` 탭을 안정적으로 운영하면서 후추·마요·짜장의 방 꾸미기·장난감·성장 보상 장면과 가계부용 행동·표정을 계속 확장한다.
- **기준 버전·원격 PR HEAD**: 로컬 작업 브랜치 v158, 원격 작업 브랜치 v156. `main`은 변경하지 않는다. 정확한 HEAD는 재개 시 로컬과 `origin/feat/phase3-mascot-v2-integration`을 각각 확인한다.
- **완료**: Phase 2K 44개 애니메이션, v136~v145 전역·9개 탭 디자인, v146~v153 저장·삭제 안전성, v154 Phase 2L 세 마리 가계부 회의·전역 무대 리디자인, v155 Phase 2M 상태별 개별 원화 7종·실제 앱 이벤트 연결, v156 `pet-v1` 아기 육성 자산과 엔진, v157 열 번째 육성 탭·로컬 자동 저장·보호 초기화·오프라인 인계, v158 일일 돌봄 3종·성장 추억 6단계·하루 3회 발바닥 찾기·기존 저장 상태 마이그레이션.
- **진행 중**: 없음. v158 검증 후 방 꾸미기·장난감 보상 또는 Phase 2N 가계부 행동 팩으로 이어간다.
- **검증 완료**: 모듈 문법, 마스코트 통합·파일·PNG/WebP 알파·6프레임 시간, 10개 탭·32개 디자인 계층, 삭제 안전성, `pet-v1` 74개 관련 파일·스키마 마이그레이션·일일 미션·성장 앨범·게임 하루 제한·앱 연결·오프라인 캐시, `git diff --check`. 기존 DOM·reduced-motion 스모크 실행에는 `linkedom` 테스트 의존성이 필요하다.
- **다음 우선순위**: 육성방 전용 방 꾸미기 슬롯, XP로 해금되는 장난감, 세대별 추억 보존, Phase 2N 가계부 행동 팩을 확장한다. 육성 수치는 계속 실제 금액·Supabase 회계 데이터와 분리한다.
- **차단 사유**: 구현 차단 없음. 원격 작업 브랜치 게시만 외부 쓰기 정책상 사용자의 명시적 허용 문구가 필요하다.
- 회계 규칙(§6), 저장 데이터의 열·값, 실제 거래 이벤트는 변경하지 않는다. 실패 확인·중복 클릭 차단·재시도만 보강한다.

---

_문의: 원저자 정근. 이 문서는 v158 작업 브랜치 기준이며, 큰 변경 시 갱신 필요._
