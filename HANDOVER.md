...
- v142 시점에는 Phase 2J~2M 복원 4커밋과 v136~v142 디자인 커밋이 로컬 작업 브랜치에 있고, Draft PR #1 원격 브랜치 반영 전일 수 있다. `git status -sb`와 원격 HEAD를 먼저 대조한다.
...
다음 화면별 디자인 작업은 월간/연간/DB 순으로, 한 화면씩 실제 데이터 가독성을 먼저 점검하며 진행한다. 회계 규칙(§6), Supabase 저장 흐름, 실제 거래 이벤트는 디자인 작업 범위에서 제외한다.
