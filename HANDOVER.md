...
- Phase 2M으로 앱에서 실제 호출되는 6개 상태의 독립 원화를 새로 제작했다: 후추 `budget_warning`·`search_no_results`, 마요 `budget_exceeded`·`history_empty`·`retry_calm`, 짜장 `goal_achieved`.
- 각 장면은 합성 시트가 아닌 개별 1024px 투명 PNG로 보존하고, 앱용 6프레임 512px WebP와 128px 정적 대체본을 각각 만들었다. 모든 파일은 캐릭터·행동별 이름으로 분리했다.
- 홈 예산이 80% 이상이면 후추의 예산 지킴이, 100% 이상이면 마요의 예산 회복 안내가 일정 시간 간격으로 자동 반응한다. 검색 결과 없음·첫 기록·저축 목표 달성·저장 재시도는 기존 실제 이벤트가 새 Phase 2M 자산을 사용한다.
...
- **완료**: Phase 2K 44개 애니메이션, v136~v145 전역·9개 탭 디자인, v146~v153 저장·삭제 안전성, v154 Phase 2L 세 마리 가계부 회의·전역 무대 리디자인, v155 Phase 2M 상태별 개별 원화 6종·애니메이션 6종·정적 대체 6종과 실제 앱 이벤트 연결.
