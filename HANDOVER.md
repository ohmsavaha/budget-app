...
> 작성 기준: **v161 작업 브랜치** (2026-08). 앱 라이브 기준본은 `main`이며, 디자인 작업은 Draft PR 브랜치에서 진행합니다.
...
- **구조**: **단일 파일 `index.html` (약 8,724줄)** 안에 전부. `<script type="module">` 하나. **프레임워크 없음, 빌드 없음.** 자체 하이퍼스크립트 `h()` 함수로 DOM 조립.
...
- **현재 목표**: Phase 2N 카테고리 반응을 안정적으로 운영하면서 지출 기록의 맥락별 행동·표정을 더 넓힌다.
- **기준 버전·원격 PR HEAD**: 작업 브랜치 v161. `main`은 변경하지 않는다. 정확한 HEAD는 재개 시 `origin/feat/phase3-mascot-v2-integration`을 확인한다.
- **완료**: Phase 2K 44개 애니메이션, v136~v145 전역·9개 탭 디자인, v146~v153 저장·삭제 안전성, v154 Phase 2L 세 마리 가계부 회의·전역 무대 리디자인, v155 Phase 2M 상태별 개별 원화 7종·실제 앱 이벤트 연결, v156 `pet-v1` 아기 육성 자산과 엔진, v157 열 번째 육성 탭, v158 일일 돌봄·성장 추억·발바닥 찾기, v159 고양이별 방 5슬롯·꾸미기 23종·XP 해금, v160 세대별 추억 자동 보관·스키마 v4 마이그레이션, v161 Phase 2N 카테고리별 지출 반응 10종.
- **진행 중**: 없음. v161 검증 후 Phase 2O의 맥락별 기록 반응 확장으로 이어간다.
- **검증 완료**: 749개 마스코트 파일, Phase 2N 10개 원화·정적 PNG 알파, 10개 WebP의 512px 캔버스·알파·6프레임 시간, 61개 앱 이벤트·70개 런타임 동작, 카테고리 매핑·금융 의미 우선순위, DOM·reduced-motion 스모크, 10개 탭·36개 디자인 계층, 삭제 안전성, 육성방 스키마 v4·76개 파일, 오프라인 캐시, `git diff --check`를 통과했다.
- **다음 우선순위**: Phase 2O에서 같은 카테고리 안에서도 첫 기록·반복 기록·큰 지출·예산 내 기록처럼 맥락이 다른 반응을 정보 가독성을 해치지 않는 범위에서 확장한다. 육성 수치와 세대 추억은 계속 실제 금액·Supabase 회계 데이터와 분리한다.
- **차단 사유**: 구현 차단 없음.
- 회계 규칙(§6), 저장 데이터의 열·값, 실제 거래 이벤트는 변경하지 않는다. 실패 확인·중복 클릭 차단·재시도만 보강한다.

---

_문의: 원저자 정근. 이 문서는 v161 작업 브랜치 기준이며, 큰 변경 시 갱신 필요._
