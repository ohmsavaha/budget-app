...
2. 고양이 이미지는 기존의 투명 PNG/마스코트 런타임 자산만 사용한다. 원본을 임의 크롭하거나 새 유사 이미지를 만들지 않는다.
...
- **현재 목표**: 9개 탭 디자인 업그레이드를 화면 단위로 완성하고, 회계·저장 로직을 건드리지 않은 채 작업 브랜치에서 검증한다.
- **기준 버전·원격 PR HEAD**: 로컬 v148, 원격 작업 브랜치 `b322d97c94d452ae8dc42028b9ee94579b203c70`(v147). `main`은 변경하지 않는다.
- **완료**: Phase 2J~2M 복원, Phase 2K 44개 애니메이션/44개 정적 대체, v136 전역 셸, v137 홈, v138 지출, v139 공용, v140 자산, v141 고정비, v142 투자, v143 월간, v144 연간, v145 품목 DB, 9개 탭 회귀 검사, v146 투자 실패 복구, v147 품목·가격 저장 안정성, v148 실제 카드대금 저장 복구.
- **진행 중**: 없음. v148 검증 후 화면 단위 커밋·게시한다.
...
- **다음 우선순위**: 9개 탭 업그레이드 완료 후 전 화면 회귀 점검. 그 다음 기존 투명 원본 자산 범위에서 세 고양이 행동·표정 확장 단계를 검토한다.
- **차단 사유**: 없음. 작업공간 정리로 이전 로컬 전용 커밋은 사라졌으므로 원격 작업 브랜치를 기준으로 재개했다.
- 회계 규칙(§6), Supabase 저장 흐름, 실제 거래 이벤트는 디자인 작업 범위에서 제외한다.

---

_문의: 원저자 정근. 이 문서는 v148 작업 브랜치 기준이며, 큰 변경 시 갱신 필요._
