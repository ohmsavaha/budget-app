...
> 작성 기준: **v159 작업 브랜치** (2026-08). 앱 라이브 기준본은 `main`이며, 디자인 작업은 Draft PR 브랜치에서 진행합니다.
...
- **현재 목표**: 실제 `🐾 육성` 탭의 방 꾸미기와 장난감 해금을 안정적으로 운영하면서 세대별 추억 보존과 가계부용 행동·표정을 계속 확장한다.
- **기준 버전·원격 PR HEAD**: 작업 브랜치 v159. `main`은 변경하지 않는다. 정확한 HEAD는 재개 시 `origin/feat/phase3-mascot-v2-integration`을 확인한다.
- **완료**: Phase 2K 44개 애니메이션, v136~v145 전역·9개 탭 디자인, v146~v153 저장·삭제 안전성, v154 Phase 2L 세 마리 가계부 회의·전역 무대 리디자인, v155 Phase 2M 상태별 개별 원화 7종·실제 앱 이벤트 연결, v156 `pet-v1` 아기 육성 자산과 엔진, v157 열 번째 육성 탭, v158 일일 돌봄·성장 추억·발바닥 찾기, v159 고양이별 방 5슬롯·꾸미기 23종·XP 해금·스키마 v3 마이그레이션.
- **진행 중**: 없음. v159 검증 후 세대별 추억 보존 또는 Phase 2N 가계부 행동 팩으로 이어간다.
- **검증 완료**: 모듈 문법, 상태 스키마 v3 마이그레이션, 5개 방 슬롯·23개 항목·XP 잠금 보호·고양이별 로컬 저장, 10개 탭·34개 디자인 계층, 삭제 안전성, 앱 연결·오프라인 캐시, `git diff --check`. 기존 719개 마스코트 자산은 변경하지 않았다. DOM·reduced-motion 스모크에는 `linkedom` 테스트 의존성이 필요하다.
- **다음 우선순위**: v160 세대별 추억 보관함과 보관 후 새 세대 시작 흐름, 이후 Phase 2N 가계부 행동 팩을 확장한다. 육성 수치는 계속 실제 금액·Supabase 회계 데이터와 분리한다.
- **차단 사유**: 구현 차단 없음.
- 회계 규칙(§6), 저장 데이터의 열·값, 실제 거래 이벤트는 변경하지 않는다. 실패 확인·중복 클릭 차단·재시도만 보강한다.

---

_문의: 원저자 정근. 이 문서는 v159 작업 브랜치 기준이며, 큰 변경 시 갱신 필요._
