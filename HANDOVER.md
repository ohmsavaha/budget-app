...
- **현재 목표**: 후추·마요·짜장의 가계부용 행동·표정 자산을 계속 확장하고, `pet-v1`을 전용 육성 탭에 연결할 때도 정보 계층과 마스코트 안전영역을 유지한다.
- **기준 버전·원격 PR HEAD**: 로컬·원격 작업 브랜치 v155 앱 + v156 육성 기반. `main`은 변경하지 않는다. 정확한 HEAD는 재개 시 `git rev-parse origin/feat/phase3-mascot-v2-integration`으로 확인한다.
- **완료**: Phase 2K 44개 애니메이션, v136~v145 전역·9개 탭 디자인, v146~v153 저장·삭제 안전성, v154 Phase 2L 세 마리 가계부 회의·전역 무대 리디자인, v155 Phase 2M 상태별 개별 원화 7종·실제 앱 이벤트 연결, v156 `pet-v1` 아기 육성 원화·애니메이션·정적 대체 각 21종과 상태·화면 기반.
- **진행 중**: 없음. v156 육성 기반 검증 후 다음 행동 팩 또는 육성 탭 연결로 이어간다.
- **검증 완료**: 모듈 문법, 마스코트 통합·파일·PNG/WebP 알파·6프레임 시간, 9개 탭 디자인 계층, 삭제 안전성, `pet-v1` 67개 파일·상태 엔진·확인 초기화, `git diff --check`. 기존 DOM·reduced-motion 스모크 실행에는 `linkedom` 테스트 의존성이 필요하다.
- **다음 우선순위**: Phase 2N 가계부 행동 팩을 계속 확장하거나, 사용자 요청 시 `pet-v1`을 별도 육성 탭으로 연결한다. 육성 연결 전에는 실제 금액·Supabase 회계 데이터와 분리한다.
...
_문의: 원저자 정근. 이 문서는 v155 작업 브랜치 기준이며, 큰 변경 시 갱신 필요._
