...
- **기준 버전·원격 PR HEAD**: 로컬·원격 작업 브랜치 v150. `main`은 변경하지 않는다. 정확한 HEAD는 재개 시 `git rev-parse origin/feat/phase3-mascot-v2-integration`으로 확인한다.
- **완료**: Phase 2J~2M 복원, Phase 2K 44개 애니메이션/44개 정적 대체, v136 전역 셸, v137 홈, v138 지출, v139 공용, v140 자산, v141 고정비, v142 투자, v143 월간, v144 연간, v145 품목 DB, 9개 탭 회귀 검사, v146 투자 실패 복구, v147 품목·가격 저장 안정성, v148 실제 카드대금 저장 복구, v149 나눠내기 생성·받음 처리 정합성, v150 일반 거래 더치페이·자동 정산 연결 복구.
- **진행 중**: 없음. v150 검증 후 단일 테이블 저장의 네트워크 예외 복구를 통일한다.
...
- **다음 우선순위**: 계좌·저축·대출 저장의 네트워크 예외 복구를 통일하고, 삭제 요청이 실패했는데 화면에서 사라지는 구간을 점검한다.
- **차단 사유**: 없음. 작업공간 정리로 이전 로컬 전용 커밋은 사라졌으므로 원격 작업 브랜치를 기준으로 재개했다.
- 회계 규칙(§6), Supabase 저장 흐름, 실제 거래 이벤트는 디자인 작업 범위에서 제외한다.

---

_문의: 원저자 정근. 이 문서는 v150 작업 브랜치 기준이며, 큰 변경 시 갱신 필요._
